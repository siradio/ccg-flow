...
const FIELDS = [
  { key: 'date', label: 'Date', hints: ['date'] },
  { key: 'product_code', label: 'Code / ID produit', hints: ['id produit', 'code', 'id'] },
  { key: 'product_designation', label: 'Désignation produit', hints: ['description', 'désignation', 'designation', 'produit'] },
  { key: 'type', label: 'Type de mouvement', hints: ['type'] },
  { key: 'quantite', label: 'Quantité', hints: ['quant', 'qté', 'qte'] },
  { key: 'localisation', label: 'Localisation', hints: ['localisation', 'entrepot', 'entrepôt', 'emplacement'] },
  { key: 'commentaire', label: 'Commentaire', hints: ['comment'] },
  { key: 'valeur', label: 'Valeur', hints: ['valeur', 'montant'] },
];
...
export default function StockImport() {
  ...
  if (!hasSubModuleLevel(user, 'stock.import')) return <div><StockSectionNav /><p>L'import de stock ne vous a pas été accordé.</p></div>;
  ...
  async function doImport() {
    ...
    try {
      ...
      setReport(rep);
    } catch (err) { setError(err.response?.data?.error || 'Échec de l\'import.'); }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 12 }}>
        <div>
          <h1 className="page-title" style={{ margin: 0 }}>Import des mouvements</h1>
          <p className="page-subtitle" style={{ margin: '4px 0 0' }}>Reprenez vos fichiers « Mvt Stock Online ». Les quantités sont prises en valeur absolue — le type porte le sens.</p>
        </div>
        <button className="btn btn-secondary" onClick={downloadTemplate}>⬇ Télécharger le modèle</button>
      </div>

      {!canImport && <div className="alert alert-warning" style={{ marginBottom: 12 }}>Vous êtes en consultation : l'import nécessite le niveau « ajout ».</div>}

      <section className="card" style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <label className="field" style={{ minWidth: 200 }}>Business Unit cible *
            <select value={buId} onChange={e => setBuId(e.target.value)} required>
              <option value="" disabled>Choisir la BU…</option>
              {bus.map(b => <option key={b.id} value={b.id}>{b.nom}</option>)}
            </select>
          </label>
          <label className="field" style={{ minWidth: 240 }}>Fichier Excel (.xlsx)
            <input type="file" accept=".xlsx,.xls,.csv" onChange={onFile} />
          </label>
          {sheets.length > 1 && (
            <label className="field" style={{ minWidth: 160 }}>Feuille
        ...
        {fileName && <p style={{ margin: '10px 0 0', fontSize: 13, color: 'var(--color-text-muted)' }}>{fileName} · {data.length} ligne(s) détectée(s)</p>}
      </section>

      {headers.length > 0 && (
        <section className="card" style={{ marginBottom: 12 }}>
          <h2 style={{ margin: '0 0 8px', fontSize: 16 }}>Correspondance des colonnes</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 10 }}>
            {FIELDS.map(f => (
              <label key={f.key} className="field">{f.label}{['type', 'quantite'].includes(f.key) ? ' *' : ''}
          ...
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginTop: 12 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 14 }}>
              <input type="checkbox" checked={opts.create_missing_products} onChange={e => setOpts(o => ({ ...o, create_missing_products: e.target.checked }))} /> Créer les produits inconnus
            </label>
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 14 }}>
              <input type="checkbox" checked={opts.create_missing_locations} onChange={e => setOpts(o => ({ ...o, create_missing_locations: e.target.checked }))} /> Créer les localisations inconnues
            </label>
          </div>
        </section>
      )}
      ...
      {mappedPreview.length > 0 && (
        <section className="card" style={{ marginBottom: 12 }}>
          <h2 style={{ margin: '0 0 8px', fontSize: 16 }}>Aperçu (8 premières lignes)</h2>
      ...
      {data.length > 0 && canImport && (
        <button className="btn btn-primary" disabled={!ready || busy} onClick={doImport}>
          {busy ? 'Import en cours…' : `Importer ${data.length} mouvement(s)`}
        </button>
      )}

      {report && (
        <section className="card" style={{ marginTop: 16 }}>
          <h2 style={{ margin: '0 0 10px', fontSize: 16 }}>Rapport d'import</h2>
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 12 }}>
            <Stat label="Total" value={report.total} />
            <Stat label="Importés" value={report.inserted} color="#15803d" />
            <Stat label="Ignorés" value={report.skipped} color={report.skipped ? '#b45309' : undefined} />
            <Stat label="Produits créés" value={report.created_products} />
            <Stat label="Localisations créées" value={report.created_locations} />
          </div>
          {report.errors?.length > 0 && (
            <>
              <h3 style={{ margin: '0 0 6px', fontSize: 14 }}>Lignes ignorées ({report.errors.length})</h3>
              <div className="table-wrap" style={{ maxHeight: 260, overflowY: 'auto' }}>
                <table><thead><tr><th>Ligne</th><th>Motif</th></tr></thead>
